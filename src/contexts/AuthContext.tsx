...
export interface User {
  ...
  name: string;
  email: string;
  role: 'director' | 'executive' | 'employee';
  avatar?: string;
  profileImage?: string;
  department?: string;
  position?: string;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (email: string, password: string): Promise<boolean> => {
    ...
    try {
      ...
      const loggedInUser: User = {
        ...
        name: result.ngoName,
        email: result.email,
        role: result.role as 'director' | 'executive' | 'employee',
      ...
      if (!users.some(u => u.email === loggedInUser.email)) {
  ...
  if (user && user.homePage) {
    const redirectPage = user.homePage;
    setUser(null);
